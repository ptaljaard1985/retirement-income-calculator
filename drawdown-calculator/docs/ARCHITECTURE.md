...
- The "Export report" button (State 2 canvas-head, alongside Auto-top-up and Lock as baseline) calls `buildReportSnapshot()`, writes the result to `localStorage['sw-drawdown-snapshot']`, and opens `retirement_drawdown_report.html` in a new tab. The snapshot is the single contract between the two files — see the report-architecture section below. This is now the canonical client-PDF path.
- The **Real | Nominal** segmented toggle (`#btn-real` / `#btn-nom`) lives on the chart-controls row alongside `Income | Capital | Table`, with `.controls-row` styled `display: flex; justify-content: space-between;` so the view selector stays on the left and the rand-framing toggle on the right.
