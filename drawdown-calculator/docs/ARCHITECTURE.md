...
- The Income chart has **five datasets** in this fixed order: `[0] LA (net)`, `[1] Disc`, `[2] Other (net)`, `[3] Target need (transparent line, data carrier)`, `[4] Tax (grey slice)`. The 4 bar datasets share `stack: 'income'`; stacking order within the stack follows array order, so Tax caps the top. `targetBoxPlugin` draws the target as a stepped top line (horizontal per-year segments spanning the full x-slot, vertical steps only where the target changes — no sides, no bottom). `shortfallShadingPlugin` paints a coral wash between the colored-bar-top and the need-line for years where net (sum of datasets 0+1+2) < target, plus a dashed vertical at the first shortfall age. Both plugins read `chart.data.datasets[3].data[i]` for the target value.
- **Tax apportionment for the Income chart** (see `incomeBarSeries()`): household `nominal.tax[i]` (or `real.tax[i]`) is apportioned to LA and Other in proportion to their gross shares; Disc is left at gross. Bar total therefore equals gross income, colored portion = net to bank, grey Tax cap = what goes to SARS. This is Option B in `docs/DESIGN.md` — the client sees the tax bite visually rather than a target line floating below the gross-bar tops.
