...
- `validateSnapshot(proj, plan, label)` — runs 12 identity assertions on a built projection block before it's emitted (Session 26 + I12 in Session 27). Throws on failure; the export-click handler at line ~6586 wraps `buildReportSnapshot()` in `try/catch` + `alert(...)` so failures surface visibly to the adviser instead of silently shipping a malformed snapshot. I12 specifically ties `sustainableTo` to the row-level `shortfall` flags so `deriveMilestones` and `buildProjectionPayload` can never silently drift apart again (Session 27 caught this with a 2-year disagreement between the report headline and the chart's coral wash).
