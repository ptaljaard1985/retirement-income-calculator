...
- The "Export report" button (State 2 canvas-head, alongside Lock as baseline) calls `buildReportSnapshot()`, writes the result to `localStorage['sw-drawdown-snapshot']`, and opens `retirement_drawdown_report.html` in a new tab. The snapshot is the single contract between the two files — see the report-architecture section below. This is now the canonical client-PDF path.
- The **chart-controls row** (`.controls-row`) carries `Income | Capital | Table` on the left; on the right, a `.controls-row-right` cluster holds the `Auto-top-up` toggle pill (`#toggle-topup-pill`) and the `Real | Nominal` mini-segmented toggle (`#btn-real` / `#btn-nom`). The pill moved here from the canvas-head action cluster in Session 10 so every chart-level mode switch sits on the same strip. `.controls-row` is `display: flex; justify-content: space-between; align-items: center;` and the cluster is `display: inline-flex; gap: 12px;`.
- The **plan-bar-lite** at the top of State 2 (`#state-single > .plan-bar-lite`) was trimmed in Session 10 to `[Simple Wealth · Retirement Drawdown] [Family <surname>] [Prepared <date>] [Edit plan ↓]`. The old `Household`, `Capital`, and `Target` facts were dropped — the outcome strip immediately below already carries the target and income mix, so surfacing them in the bar too was redundant. `#pb-family` is written by `updatePlanBarLite()` (no-arg helper) from the State-1 title-plate span `#hl-family.textContent`. The call site in `refresh()` passes no argument.
